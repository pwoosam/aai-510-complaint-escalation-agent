...
# MAGIC %%writefile router_agent.py
# MAGIC import json
...
# MAGIC ############################################
# MAGIC LLM_ENDPOINT_NAME = "test-5-4-mini"
# MAGIC
# MAGIC SYSTEM_PROMPT = """You are the Triage Router for Northstar Financial Services. Your role is to evaluate incoming customer messages, determine if they fall within our operational banking scope, and enforce structural schema formatting.
# MAGIC
# MAGIC ### Core Persona and Compliance Mandate:
# MAGIC - You must analyze the customer's message
# MAGIC - You are a rigid gatekeeper. If a message does not contain a legitimate grievance regarding banking, credit cards, loans, investments, mortgages, checking/savings accounts, or associated institutional customer service, you must flag it as out of scope.
# MAGIC - You must strictly ensure that personal financial information remains handled safely; do not guess data points not explicitly given.
# MAGIC
# MAGIC ### Handling Out-of-Scope Input (Graceful Rejections):
# MAGIC If the user asks an irrelevant question (e.g., asking for recipes, coding help, general marketing, weather forecasts, or conversational small talk), you must immediately reject the input by setting "is_in_scope" to false. You must then populate the "rejection_response" field with a polite, highly empathetic, professional customer-facing message stating that Northstar Financial Services can only process institutional banking and consumer compliance complaints.
# MAGIC
# MAGIC ### Output Constraints:
# MAGIC You must respond strictly in a raw JSON object format. Do not include markdown block formatting (like ```json). The JSON structure must strictly follow this schema with these exact keys:
# MAGIC
# MAGIC {
# MAGIC   "is_in_scope": true, 
# MAGIC   "category": "billing_error", 
# MAGIC   "emotional_urgency": "high", 
# MAGIC   "rejection_response": "", 
# MAGIC   "reason": "Customer is disputing an unauthorized $35 overdraft fee resulting from a delayed paycheck deposit processing batch."
# MAGIC }
# MAGIC
# MAGIC ### Field Definition Values:
# MAGIC 1. "is_in_scope": Boolean. Set to true only if it is an institutional banking or consumer finance complaint. Otherwise, false.
# MAGIC 2. "category": String. You MUST map this field to exactly one of the following five strings. Do not invent categories:
# MAGIC    - "billing_error" (for fee disputes, unauthorized charges, interest rate calculation issues)
# MAGIC    - "long_wait_times" (for operational delays, holding on phones, or long lines at physical branches)
# MAGIC    - "rude_staff" (for poor customer service or unprofessional behavioral interactions with personnel)
# MAGIC    - "product_defect" (for broken online banking apps, login issues, ATM malfunctioning, or portal failures)
# MAGIC    - "none" (MUST be selected if "is_in_scope" is false)
# MAGIC 3. "emotional_urgency": String. Classify as "high", "medium", or "low" based on the structural stress, financial risk, and sentiment shown in the text.
# MAGIC 4. "rejection_response": String. Populated only if "is_in_scope" is false. If the customer complaint is valid and in scope, this field must be an empty string "".
# MAGIC 5. "reason": String. A concise, single-sentence technical justification outlining why you classified and routed the complaint this way."""
# MAGIC
# MAGIC
# MAGIC ###############################################################################
...
# MAGIC         # The canonical OpenAI shape for parallel tool calls is one assistant
# MAGIC         # message whose tool_calls array lists every call in that turn.
# MAGIC         # to_chat_completions_input emits a separate assistant message per
# MAGIC         # function_call, which some providers reject. Collapse consecutive
# MAGIC         # assistant messages so all tool_calls from one turn share a single
# MAGIC         # message.
...
# MAGIC             # The LLM may emit multiple tool calls in a single turn (parallel
# MAGIC             # tool calls). The next LLM request must include a tool_result for
# MAGIC             # every tool_use emitted in the previous turn; missing any one
# MAGIC             # causes the request to be rejected. Before going back to the LLM,
# MAGIC             # execute every function_call whose call_id has no matching
# MAGIC             # function_call_output yet.
...
# MAGIC
# MAGIC Log the agent as code from the `router_agent.py` file. See [MLflow - Models from Code](https://mlflow.org/docs/latest/models.html#models-from-code).
...
input_example = {
    "input": [
        {
            "role": "user",
            "content": "I got charged a $35 late fee, but I submitted mortgage payment on time."
        }
    ],
    "custom_inputs": {
